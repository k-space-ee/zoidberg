...
def function_fit(func, X, Y, maxfev=1000, debug=False, initial_guess=None):
    start = time()
    constants, _ = scipy.optimize.curve_fit(func, np.array(X), np.array(Y), p0=initial_guess, maxfev=maxfev)
    ...
    if debug:
        ...
        print("vars:", ", ".join(str(c) for c in constants))
        print("average delta:", delta / len(X))
